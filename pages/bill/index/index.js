...
Page({
  ...
  getData: function () {
    Promise.all([this.QueryStatus(), this.QueryMonthlyDashBoard()]).then(res => {
  ...
  QueryStatus: function () {
    ...
    util.request(api.BillApi.QueryStatus, {
      ...
      my.setStorageSync({ key: "curr_status", data: res.data.recv_status });
      my.setStorageSync({ key: "is_verify_cust_confirm", data: res.data.is_verify_cust_confirm });
  ...
  Affirm: function (e) {
    ...
    util.request(api.BillApi.PostAffirm, {
      ...
      if (res.data.code == "0") {
        ...
        if (msg[14] < 0) {
          if (msg[14] == -1) {
            util.Toast("您本月委托的人事代理服务还未完成,待人事代理完成后再进行汇总确认,谢谢!")
            return;
          }
          msg[14] = 0;
        }
        ...
        my.confirm({
          title: '温馨提示',
          content: '请先关注微信公众号,方便给您推送做账消息通知',
  ...
  AffirmPay: function (e) {
    ...
    if (est_service_amt > 0) {
      ...
      if (!isbalance_amt) {
        ...
        util.request(api.BillApi.PostPayTrans, {
          ...
          if (res.data.success == true) {
            ...
            that.goPay(pay_trans_id, '记账费用(' + curr_month + ')', '2021002119655215');
          }
        })
      }
      //余额支付
      ...
      util.Toast("服务费用估算异常,请联系服务人员核对后确认支付");
    }
  },
