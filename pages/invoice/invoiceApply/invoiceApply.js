...
Page({
  ...
  bindInvoiceItemTypeChange: function (e) {
    this.setData({
      invoice_item_type: this.data.info.listInvoiceItemType[e.detail.value].code_name,
    })
    this.GetTaxRate();
  },
  ...
  postData: function () {
    ...
    if (that.data.invoice_type != '增值税电子普通发票' && that.data.invoice_type != '增值税电子专用发票') {
      ...
      if (that.data.to_email == null || that.data.to_email == undefined || that.data.to_email.length <= 0) {
        util.Toast("电子发票发送邮件地址为空,请先认证个人邮件!");
        return;
      }
    }
      ...
      success: function (sm) {
        if (sm.confirm) {
          ...
          util.request(api.BillApi.PostInvoiceSbmitNoPay, data, 'POST').then(function (res) {
            if (res.data.success == true) {
              util.Toast("您的开票需求已经提交，我们会尽快处理，谢谢！", 'success');
              that.goToPage();
            } else {
              util.Toast(res.data.msg, 'fail');
            }
