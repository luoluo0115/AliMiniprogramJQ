...
Page({
  data: {
    tabs2: [{ title: '增值税',},{ title: '工资薪金',},{ title: '非居民工资',},{ title: '劳务报酬',},{ title: '年终奖金',},{ title: '生产经营',}],
    activeTab2: 0,
    index1: 0,
    index2: 0,
    index4: 0,
  ...
  onTxtIncomeChange(event) {//输入当月工资
    let that = this;
    let txtIncomeValue = event.detail.value
     console.log(txtIncomeValue,'输入当月工资')
    that.setData({
      txtIncomeValue: txtIncomeValue,
      totalIncomeValue: txtIncomeValue * that.data.totalMonth
    })
  },
  onTxtInsureChange(event) {//输入五险一金
    let that = this;
    let txtInsureValue = event.detail.value
    console.log(txtInsureValue,'输入五险一金')
    that.setData({
      txtInsureValue: txtInsureValue,
      totalInsureValue: txtInsureValue * that.data.totalMonth
    })
  },
  onTxtSpecialChange(event) {
    let that = this;
    let txtSpecialValue = event.detail.value
    console.log(txtSpecialValue,'txtSpecialValue')
    that.setData({
      txtSpecialValue: txtSpecialValue,
      totalSpecialValue: txtSpecialValue * that.data.totalMonth
    })
  },
  formWageTaxSubmit: function (e) {
     let that = this;
    let txtIncome = that.data.txtIncomeValue;//本月税前工资
    let txtInsure = that.data.txtInsureValue;//本月五险一金
    let txtSpecial = that.data.txtSpecialValue;//本月专项附加扣除
    let txtBaseLine = that.data.txtBaseLine;//减除费用(起征点)
    let totalIncome = that.data.totalIncomeValue;//累计收入
    let totalInsure = that.data.totalInsureValue;//累计五险一金
    let totalSpecial = that.data.totalSpecialValue;//累计专项扣除
    let totalBaseLine = that.data.txtBaseLine* that.data.totalMonth;//累计减除费用
  ...
  formNonWageTaxSubmit: function (e) {
    ...
    let txtNonBaseLine = that.data.txtNonBaseLine;//减除费用(起征点)
  ...
  onChangeSwitch(event) {
    console.log(event,'收入含税')
    this.setData({
      checked: event.detail.value
    });
  },
  onClickAmt:function(event){
    this.setData({
      invoice_amt: event.detail.value
    });
  },  
  formSubmit: function (e) {
    ...
    let invoice_amt = Number(that.data.invoice_amt);//总额
    let tax_rate = that.data.tax_rate[that.data.index4].replace(/%/g, '') / 100;
    let enterprise_type = that.data.enterprise_type[that.data.index1];
    let declarer_type = that.data.declarer_type[that.data.index2];
    let invoice_type = that.data.invoice_type[that.data.index3];
    let postData={
    ...
    if (invoice_amt == '') {
      ...
      //税金计算器
      util.request(api.CalculateTaxUrl,postData, 'POST').then(function (res) {
